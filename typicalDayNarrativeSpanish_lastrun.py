...
expName = 'typical-day'  # from the Builder filename that created this script
expInfo = {
    'Nombre': '',
    'Edad': '',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    instructions_text = visual.TextStim(win=win, name='instructions_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    start_rec_keypress = keyboard.Keyboard()
    
    # --- Initialize components for Routine "recording" ---
    mic = visual.ImageStim(
        win=win,
        name='mic', 
    ...
    stop_rec_keypress = keyboard.Keyboard()
    ...
    end_text = visual.TextStim(win=win, name='end_text',
        ...
        languageStyle='LTR',
        depth=0.0);
    exit_exp_keypress = keyboard.Keyboard()
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    pulses_responseComponents = [text, key_resp_2]
    ...
    continueRoutine = True
    # update component parameters for each repeat
    start_rec_keypress.keys = []
    start_rec_keypress.rt = []
    _start_rec_keypress_allKeys = []
    # keep track of which components have finished
    instructionsComponents = [instructions_text, start_rec_keypress]
    ...
    while continueRoutine:
        ...
        # *start_rec_keypress* updates
        
        # if start_rec_keypress is starting this frame...
        if start_rec_keypress.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            start_rec_keypress.frameNStart = frameN  # exact frame index
            start_rec_keypress.tStart = t  # local t and not account for scr refresh
            start_rec_keypress.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(start_rec_keypress, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('start_rec_keypress.started', t)
            # update status
            start_rec_keypress.status = STARTED
            # keyboard checking is just starting
            start_rec_keypress.clock.reset()  # now t=0
            start_rec_keypress.clearEvents(eventType='keyboard')
        if start_rec_keypress.status == STARTED:
            theseKeys = start_rec_keypress.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _start_rec_keypress_allKeys.extend(theseKeys)
            if len(_start_rec_keypress_allKeys):
                start_rec_keypress.keys = _start_rec_keypress_allKeys[-1].name  # just the last key pressed
                start_rec_keypress.rt = _start_rec_keypress_allKeys[-1].rt
                start_rec_keypress.duration = _start_rec_keypress_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in instructionsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if start_rec_keypress.keys in ['', [], None]:  # No response was made
        start_rec_keypress.keys = None
    thisExp.addData('start_rec_keypress.keys',start_rec_keypress.keys)
    if start_rec_keypress.keys != None:  # we had a response
        thisExp.addData('start_rec_keypress.rt', start_rec_keypress.rt)
        thisExp.addData('start_rec_keypress.duration', start_rec_keypress.duration)
    thisExp.nextEntry()
    ...
    stop_rec_keypress.keys = []
    stop_rec_keypress.rt = []
    _stop_rec_keypress_allKeys = []
    # keep track of which components have finished
    recordingComponents = [mic, stop_rec_keypress]
    ...
    while continueRoutine:
        ...
        # *stop_rec_keypress* updates
        
        # if stop_rec_keypress is starting this frame...
        if stop_rec_keypress.status == NOT_STARTED and t >= 5.0-frameTolerance:
            # keep track of start time/frame for later
            stop_rec_keypress.frameNStart = frameN  # exact frame index
            stop_rec_keypress.tStart = t  # local t and not account for scr refresh
            stop_rec_keypress.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(stop_rec_keypress, 'tStartRefresh')  # time at next scr refresh
            # update status
            stop_rec_keypress.status = STARTED
            # keyboard checking is just starting
            stop_rec_keypress.clock.reset()  # now t=0
        if stop_rec_keypress.status == STARTED:
            theseKeys = stop_rec_keypress.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _stop_rec_keypress_allKeys.extend(theseKeys)
            if len(_stop_rec_keypress_allKeys):
                stop_rec_keypress.keys = _stop_rec_keypress_allKeys[-1].name  # just the last key pressed
                stop_rec_keypress.rt = _stop_rec_keypress_allKeys[-1].rt
                stop_rec_keypress.duration = _stop_rec_keypress_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # check responses
    if stop_rec_keypress.keys in ['', [], None]:  # No response was made
        stop_rec_keypress.keys = None
    thisExp.addData('stop_rec_keypress.keys',stop_rec_keypress.keys)
    if stop_rec_keypress.keys != None:  # we had a response
        thisExp.addData('stop_rec_keypress.rt', stop_rec_keypress.rt)
        thisExp.addData('stop_rec_keypress.duration', stop_rec_keypress.duration)
    thisExp.nextEntry()
    ...
    exit_exp_keypress.keys = []
    exit_exp_keypress.rt = []
    _exit_exp_keypress_allKeys = []
    # keep track of which components have finished
    aknowledgmentComponents = [end_text, exit_exp_keypress]
    ...
    while continueRoutine:
        ...
        # *exit_exp_keypress* updates
        
        # if exit_exp_keypress is starting this frame...
        if exit_exp_keypress.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            exit_exp_keypress.frameNStart = frameN  # exact frame index
            exit_exp_keypress.tStart = t  # local t and not account for scr refresh
            exit_exp_keypress.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(exit_exp_keypress, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('exit_exp_keypress.started', t)
            # update status
            exit_exp_keypress.status = STARTED
            # keyboard checking is just starting
            exit_exp_keypress.clock.reset()  # now t=0
            exit_exp_keypress.clearEvents(eventType='keyboard')
        if exit_exp_keypress.status == STARTED:
            theseKeys = exit_exp_keypress.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _exit_exp_keypress_allKeys.extend(theseKeys)
            if len(_exit_exp_keypress_allKeys):
                exit_exp_keypress.keys = _exit_exp_keypress_allKeys[-1].name  # just the last key pressed
                exit_exp_keypress.rt = _exit_exp_keypress_allKeys[-1].rt
                exit_exp_keypress.duration = _exit_exp_keypress_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    # check responses
    if exit_exp_keypress.keys in ['', [], None]:  # No response was made
        exit_exp_keypress.keys = None
    thisExp.addData('exit_exp_keypress.keys',exit_exp_keypress.keys)
    if exit_exp_keypress.keys != None:  # we had a response
        thisExp.addData('exit_exp_keypress.rt', exit_exp_keypress.rt)
        thisExp.addData('exit_exp_keypress.duration', exit_exp_keypress.duration)
    thisExp.nextEntry()
    ...
    first_row_df = df[[True, False, False]]
